\levelstay{Linear Transformations and their matrices}
...
\leveldown{Linear transformations}
...
You know of many linear transformations from physics.
...
\begin{definition}[Inner product]
  An \newword{inner product} on a (real) vector space $\VS$ is a function which takes two vectors $\ket{v}$ and $\ket{w}$ and returns a real number.
  ...
  \end{enumerate}
\end{definition}
The number $\braket{v}{v}$ is often denoted $\norm{v}^2$ and is called the \newword{norm squared} of $\ket{v}$.

In the case of vectors in $\reals^n$, the standard dot product $\textbf{v}\cdot \textbf{w}$ is an inner product.
...
The terms \newword{bra} and \newword{ket} suggest the term \newword{braket} for the symbol $\braket{v}{w}$.
...
\levelstay{Inner products - Exercises}
\begin{itemize}
...
Given a linear transformation $T$ and a basis $e$ we denote the $ij^{th}$ \newword{matrix element} of $T$ in this basis by $T^e_{ij}$.
...
\levelstay{Matrix elements - exercises}
\begin{itemize}
...
\levelstay{Dual transformations}
...
It is alternately called the \newword{dual} of $T$, the \newword{adjoint} of $T$, the \newword{conjugate} of $T$, or as \newword{$T$ dagger}.
...
\levelstay{Dual transformations - Exercises}
\begin{itemize}
...
\levelstay{Symmetric transformations}
A linear transformation on a real vector space that is equal to its own dual is called \newword{symmetric}.
...
\levelstay{Eigenvectors and eigenvalues}
...
\begin{definition}[Eigenvector and Eigenvalue]
  An \newword{eigenvector} of a transformation $T$ is a vector $\ket{v}$ such that \mbox{$T\ket{v} = \lambda \ket{v}$} where $\lambda$ is a scalar.  $\lambda$ is called the \newword{eigenvalue of $\ket{v}$ with respect to $T$}.
\end{definition}
...
In fact, finding a basis that diagonalizes a linear transformation is one of the most basic steps in solving an enormous range of physics problems.
...
\levelstay{Eigenvectors and eigenvalues - Exercises}
\begin{itemize}
...
Now we know how to switch from one basis $e$ to another basis $f$ as long as we know the components of the ``new'' basis vectors $\ket{f_i}$ with respect to the ``old'' basis $f$.
...
The other difficulty was that whereas we showed that it's easy to compute $[U]^e$, we don't immediatly know how to compute $[U]^f$.
...
\levelstay{Changing basis - Exercises}
\begin{enumerate}
